...
describe('ValidationServiceV2', () => {
  ...
  describe('validateTransferRequest', () => {
    ...
    it('should throw ValidationError when to is missing', () => {
      ...
      expect(() => validationServiceV2.validateTransferRequest(invalidRequest)).toThrow(ValidationError);
      expect(() => validationServiceV2.validateTransferRequest(invalidRequest)).toThrow(
        'Missing required fields (to, UPC, quantity, rule).'
      );
    });
    ...
    it('should throw ValidationError for invalid source warehouse when provided', () => {
      ...
      expect(() => validationServiceV2.validateTransferRequest(invalidRequest)).toThrow(ValidationError);
      expect(() => validationServiceV2.validateTransferRequest(invalidRequest)).toThrow(
        'Invalid source warehouse. Valid options: WH1, WH2'
      );
    });
    ...
    it('should throw ValidationError for invalid destination warehouse', () => {
      ...
      expect(() => validationServiceV2.validateTransferRequest(invalidRequest)).toThrow(ValidationError);
      expect(() => validationServiceV2.validateTransferRequest(invalidRequest)).toThrow(
        'Invalid destination warehouse. Valid options: WH1, WH2'
      );
    });
    ...
    it('should throw ValidationError for invalid transfer rule', () => {
      ...
      expect(() => validationServiceV2.validateTransferRequest(invalidRequest)).toThrow(ValidationError);
      expect(() => validationServiceV2.validateTransferRequest(invalidRequest)).toThrow(
        'Invalid transfer rule. Valid options: fastest, cheapest'
      );
    });
  ...
  describe('validateWarehouseRegistration', () => {
    ...
    it('should throw ValidationError when id is missing', () => {
      ...
      expect(() => validationServiceV2.validateWarehouseRegistration(invalidConfig)).toThrow(ValidationError);
      expect(() => validationServiceV2.validateWarehouseRegistration(invalidConfig)).toThrow(
        'Missing required fields (id, name, location, api).'
      );
    });
    ...
    it('should throw ValidationError when warehouse ID already exists', () => {
      ...
      expect(() => validationServiceV2.validateWarehouseRegistration(validWarehouseConfig)).toThrow(ValidationError);
      expect(() => validationServiceV2.validateWarehouseRegistration(validWarehouseConfig)).toThrow(
        'Warehouse with ID "NEW_WH" already exists.'
      );
    });
  ...
  describe('dynamic warehouse validation', () => {
    ...
    it('should reject requests to warehouses that have been removed', () => {
      ...
      expect(() => validationServiceV2.validateTransferRequest(requestToRemovedWarehouse)).toThrow(ValidationError);
      expect(() => validationServiceV2.validateTransferRequest(requestToRemovedWarehouse)).toThrow(
        'Invalid destination warehouse. Valid options: WH1'
      );
    });
  });
});
